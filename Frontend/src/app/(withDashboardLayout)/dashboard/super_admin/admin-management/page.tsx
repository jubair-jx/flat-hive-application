...
function UserManagementPage() {
  ...
  const handleOnUpdate = async (
    ...
    const res = await updateAdminInfo(payload).unwrap();
    await refetch();
    if (res?.id) {
  ...
  return (
    <section>
      <div className="flex justify-between my-4">
        <CommonDashboardHeaderTitle mainTitle="Admin Management" />
        <button
          onClick={() => setModalOpen(true)}
          className=" px-4 py-1 text-sm font-poppins font-medium bg-blue-600 text-white rounded-md"
        >
          Create Admin
        </button>
      </div>
      ...
      <div className="w-full mb-8 overflow-hidden rounded-lg shadow-xs mt-10">
        <div className="w-full overflow-x-auto">
          <table className="w-full whitespace-no-wrap">
            <thead>
              <tr className="text-xs font-semibold tracking-wide text-gray-800 font-poppins uppercase border-b bg-violet-200 ">
      ...
      <CreateAdminModal ModalOpen={isModalOpen} setModalOpen={setModalOpen} />
    </section>
  );
}
